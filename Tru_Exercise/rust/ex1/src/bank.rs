...
impl<T: PartialOrd + Display, U: Debug> Bank<T, U> {
    ...
    pub fn deposit_lock_box(&mut self, customer: &str, item: U) {
        ...
        (*l_box).push(item);
        println!(
            "{customer}, your item is now locked away\n - {}",
            self.banker
        );
    ...
    /*pub fn withdrawal(&mut self, customer: &str, amount: T) {
        let mut i = 0;
        for (customer_identifier, lock_box_identifier) in &self.lock_box {
            if customer_identifier == customer {
                if self.vault[i] > amount {
